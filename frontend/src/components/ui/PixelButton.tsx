...
export function PixelButton({
  ...
  return (
    <motion.button
      ...
      onClick={onClick}
      disabled={disabled}
      className={`font-pixel border-4 ${VARIANT_STYLES[variant]} ${SIZE_STYLES[size]} ${
        disabled ? 'cursor-not-allowed opacity-50' : 'cursor-pointer'
      } ${className} relative transition-colors duration-100 active:left-[2px] active:top-[2px]`}
      style={{ boxShadow: disabled ? '2px 2px 0px 0px #666' : '4px 4px 0px 0px #101010' }}
    >
      {children}
    </motion.button>
  );
